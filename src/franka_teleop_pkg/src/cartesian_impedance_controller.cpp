...
controller_interface::CallbackReturn CartesianImpedanceController::on_init() {
  try {
    ...
    auto_declare<double>("translational_stiffness", translational_stiffness_); //150
    auto_declare<double>("rotational_stiffness", rotational_stiffness_); //10
...
//?
  ...
  std::cout << "INITIAL" << q_initial << std::endl; // todo
...
controller_interface::return_type CartesianImpedanceController::update(
  ...
  const double position_deadband_threshold = 8e-3;  // in meters (adjust as needed)
  ...
  const double orientation_deadband_threshold = 2e-2;  // in radians (adjust as needed)
...
//  tau_task = jacobian.transpose() * (-cartesian_stiffness_ * position_error - cartesian_damping_ * jacobian * dq); //!
  ...
  static constexpr size_t ramp_duration = 5000; // ~0.5s at 1000Hz
